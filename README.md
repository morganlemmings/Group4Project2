...
# Manipulations 
To prepare the dataset for analysis in Tableau, several key manipulations were performed to ensure data quality and enable meaningful visualizations. First, rows with missing or null values in the ForkLength column were manually deleted from the Excel file prior to import, ensuring that all analyses were based on valid measurements and preventing errors in Tableau. The Date field was converted to a continuous format to support time-based analysis, with dates optionally aggregated by month or week using Tableau’s built-in hierarchy for smoother trend visualization. Fork length values were consistently aggregated using the average function to reduce the influence of outliers and allow for clear comparisons across reaches and species. To explore spatial and biological variation, the Fork and Species fields were used to group and color data in all graphs. Linear regression trend lines were added to line plots via Tableau’s Analytics Pane to quantify the relationship between fork length and date, with settings configured to display equations and R² values and fit separate models per reach or species. Additionally, Tableau’s forecasting tool was applied to time series charts to project expected fork lengths beyond the observed sampling period, with forecast length and confidence intervals adjusted to suit the dataset. 
